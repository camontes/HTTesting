...
internal sealed class GetInductionSentQueryHandler(
    ...
    ) : IRequestHandler<GetInductionSentQuery, ErrorOr<List<InductionSentResponse>>>
{
    ...
    public async Task<ErrorOr<List<InductionSentResponse>>> Handle(GetInductionSentQuery query, CancellationToken cancellationToken)
    {
        ...
        return finalResponse.OrderByDescending(x => x.DetailInductionList.Max(i => i.ShipmentDateFormat)).ToList();
    }
}
