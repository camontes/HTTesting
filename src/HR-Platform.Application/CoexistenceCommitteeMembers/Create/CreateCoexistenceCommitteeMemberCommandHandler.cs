...
internal sealed class CreateCoexistenceCommitteeMembersCommandHandler(
    ICollaboratorRepository collaboratorRepository,
    IUnitOfWork unitOfWork
    ) : IRequestHandler<CreateCoexistenceCommitteeMembersCommand, ErrorOr<bool>>
{
    private readonly ICollaboratorRepository _collaboratorRepository = collaboratorRepository ?? throw new ArgumentNullException(nameof(collaboratorRepository));
    private readonly IUnitOfWork _unitOfWork = unitOfWork ?? throw new ArgumentNullException(nameof(unitOfWork));

    public async Task<ErrorOr<bool>> Handle(CreateCoexistenceCommitteeMembersCommand command, CancellationToken cancellationToken)
    {
        ...
        if (command.CollaboratorIds is not null && command.CollaboratorIds.Count > 0)
        {
            ...
            {
                collaborator.IsCoexistenceCommitteeMember = true;
