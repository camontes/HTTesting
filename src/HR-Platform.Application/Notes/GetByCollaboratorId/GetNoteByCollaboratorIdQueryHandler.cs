...
internal sealed class GetNoteByCollaboratorIdQueryHandler(
    ...
    ) : IRequestHandler<GetNoteByCollaboratorIdQuery, ErrorOr<List<NotesResponse>>>
{
    ...
    //Desde HV por TH  
    public async Task<ErrorOr<List<NotesResponse>>> Handle(GetNoteByCollaboratorIdQuery query, CancellationToken cancellationToken)
    {
       ...
       : noteList.Where(n => !n.IsPublic && n.Assignee.BusinessEmail.Value == oldCollaborator.BusinessEmail.Value).ToList(); // Privadas 

        var mainNotes = filteredNotes.Where(n => n.ParentNoteId == null).ToList();

        var response = mainNotes
            .Select(mainNote => MapNoteToResponse(mainNote, filteredNotes, query.EmailWhoLogIn))
    ...
    private NotesResponse MapNoteToResponse(Note note, List<Note> allResponses, string email)
    {
        ...
        return new NotesResponse(
            ...
            email == note.Creator.BusinessEmail.Value,
            filesList,
            answers
        );
    }
