...
[Route("CurrencyTypes")]
public class CurrencyTypes(ISender mediator) : ApiController
{
    ...
    public async Task<IActionResult> GetAllNames()
    {
        ...
        ErrorOr<IReadOnlyList<CurrencyTypesResponse>> CurrencyTypesResult = await _mediator.Send(new GetAllCurrencyTypesQuery());

        return CurrencyTypesResult.Match(
            CurrencyTypes => Ok(CurrencyTypes),
            errors => Problem(errors)
        );
    }
}