...
[Route("StudyTypes")]
public class StudyTypes(ISender mediator) : ApiController
{
    private readonly ISender _mediator = mediator ?? throw new ArgumentNullException(nameof(mediator));

    [HttpGet("GetStudyTypes")]
    public async Task<IActionResult> GetAllNames()
    {
        ...
        ErrorOr<IReadOnlyList<StudyTypesResponse>> StudyTypesResult = await _mediator.Send(new GetAllStudyTypesQuery());

        return StudyTypesResult.Match(
            StudyTypes => Ok(StudyTypes),
            errors => Problem(errors)
        );
    }
}