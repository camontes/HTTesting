...
public class OccupationalRecommendationConfiguration : IEntityTypeConfiguration<OccupationalRecommendation>
{
    public void Configure(EntityTypeBuilder<OccupationalRecommendation> builder)
    {
        string[] formats = ["MM/dd/yyyy HH:mm:ss", "yyyy/MM/dd HH:mm:ss", "yyyy-MM-dd HH:mm:ss"];

        builder.HasKey(c => c.Id);
        builder.Property(c => c.Id).HasConversion(
            DefaultOccupationalRecommendationId => DefaultOccupationalRecommendationId.Value,
            value => new OccupationalRecommendationId(value)
        );

        builder.HasOne(p => p.Collaborator).WithMany(c => c.OccupationalRecommendations).HasForeignKey(c => c.CollaboratorId);
