...
public class WorkplaceEvidenceConfiguration : IEntityTypeConfiguration<WorkplaceEvidence>
{
    public void Configure(EntityTypeBuilder<WorkplaceEvidence> builder)
    {
        string[] formats = ["MM/dd/yyyy HH:mm:ss", "yyyy/MM/dd HH:mm:ss", "yyyy-MM-dd HH:mm:ss"];

        builder.HasKey(c => c.Id);
        builder.Property(c => c.Id).HasConversion(
            DefaultWorkplaceEvidenceId => DefaultWorkplaceEvidenceId.Value,
            value => new WorkplaceEvidenceId(value)
        );

        builder.HasOne(p => p.Collaborator).WithMany(c => c.WorkplaceEvidences).HasForeignKey(c => c.CollaboratorId);
