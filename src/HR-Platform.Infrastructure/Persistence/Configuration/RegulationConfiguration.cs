...
public class RegulationConfiguration : IEntityTypeConfiguration<Regulation>
{
    public void Configure(EntityTypeBuilder<Regulation> builder)
    {
        ...
        builder.Property(c => c.Id).HasConversion(
            DefaultRegulationId => DefaultRegulationId.Value,
            value => new RegulationId(value)
        );

        builder.HasOne(p => p.Company).WithMany(c => c.Regulations).HasForeignKey(c => c.CompanyId);
